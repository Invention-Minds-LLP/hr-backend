 ...
 */
function recommended(key, fallback = "") {
    const val = process.env[key];
    if (!val || !val.trim()) {
...
function validateConfig() {
    if (missingRecommended.length) {
        console.warn(`⚠️  [config] Recommended env vars not set (related features will fail): ${missingRecommended.join(", ")}`);
    }
    ...
    console.log(`✅ [config] Loaded for client "${exports.config.clientId}" (env: ${exports.config.nodeEnv})`);
}
