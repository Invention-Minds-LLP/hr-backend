...
export interface OfferLetterData {
  ...
  ctc?: number | null;          // Annual CTC in INR
  joinLocation?: string | null;
  workMode?: string | null;     // OFFICE / REMOTE / HYBRID
  proposedJoinAt?: Date | null;
  customNotes?: string | null;
  // Company / sender branding — driven by env so a single change reflects on every letter
  companyName?: string;
...
export function generateOfferLetterPdf(data: OfferLetterData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (address) doc.moveDown(0.2).fontSize(10).fillColor("#444").text(address, { align: "center" });
      doc.moveDown(0.6);
      ...
      doc.fontSize(11).fillColor("#000").text(
        ...
        `you will make a valuable contribution to our team.`,
        { align: "justify" }
      );
      doc.moveDown(0.8);
      ...
      doc.moveDown(0.3);
      doc.fontSize(11).fillColor("#000").text(
        `Please confirm your acceptance by signing this offer through the candidate portal ` +
        ...
        `and submission of all required documents.`,
        { align: "justify" }
      );
      doc.moveDown(0.6);

      doc.fontSize(11).text(
        `We are excited about the prospect of you joining our organization and look forward to ` +
        `your positive response. Should you have any questions about this offer, please feel free ` +
        `to reach out to the Human Resources team.`,
        { align: "justify" }
      );
      ...
      doc.fontSize(8).fillColor("#888").text(
        "This is a system-generated offer letter. The terms above supersede any prior verbal or " +
        "written communication regarding employment with the company.",
        { align: "center" }
      );
