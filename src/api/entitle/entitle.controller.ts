...
import { PrismaClient, LeaveStatus, WFHStatus, PermissionStatus } from '@prisma/client';

const prisma = new PrismaClient();

// GET all entitlement policies
export const getAllEntitlementPolicies = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const getEntitlementPolicyByYear = async (req: Request, res: Response): Promise<void> => {
...
export const getEmployeeUsageSummary = async (req: Request, res: Response) => {
    try {
        const employees = await prisma.employee.findMany({
            include: {
                ...
                WFHRequest: {
                    where: { status: WFHStatus.APPROVED },
                },
                permissions: {
        ...
        const summary = employees.map(emp => {
            ...
            const totalWFHDays = emp.WFHRequest.reduce((sum, wfh) => {
                const days = Math.ceil(
                    (wfh.endDate.getTime() - wfh.startDate.getTime()) / (1000 * 60 * 60 * 24)
                ) + 1;
                return sum + days;
            }, 0);
...
export const getEmployeeRequests = async (req: Request, res: Response) => {
  ...
  function permissionHours(
    ...
    timing?: 'FULLDAY' | 'HALFDAY' | 'HOURLY' | null
  ): number {
    if (start && end) {
  ...
  function prorateByDOJ(
    doj: Date,
    policy: { leaveEntitlement: number; wfhEntitlement: number; permissionEntitlement: number },
    year: number
  ) {
    ...
    const perMonth = (n: number) => (n / 12) * months;
